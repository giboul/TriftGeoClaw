...
def parse_args():
    ...
    parser.add_argument("-a", "--avac", type=str, nargs="?", default="_output")
    parser.add_argument("-t", "--tsul", type=str, nargs="?", default="_output")
...
args = parse_args()
TSULdir = projdir / "tsul" / args.tsul
AVACdir = projdir / "avac" / args.avac

TSULdata = read_clawdata(TSULdir / "fgout_grids.data", sep="#", skiprows=7)
AVACdata = read_clawdata(AVACdir / "fgout_grids.data", sep="#", skiprows=7)
...
for i in range(AVACdata["nout"]):
    t = read_clawdata(AVACdir/f"fgout0001.t{i+1:0>4}", sep=" ")["time"]
    AVACtimes.append(t)
...
for i in range(TSULdata["nout"]):
    t = read_clawdata(TSULdir/f"fgout0001.t{i+1:0>4}", sep=" ")["time"]
    TSULtimes.append(t)
