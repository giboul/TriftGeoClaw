...
% Ce résultat est important car les solveurs numériques des équations de Saint-Venant ont pour inconnue le vecteur $\bm q_n\mathrm dA = (h, hu, hv)^\intercal~\mathrm dA$; la masse volumique constante est retranchée au vecteur des inconnues $\bm q=\varrho \bm q_n$. Changer la masse volumique et migrer les quantités $\bm q$ d'un modèle à l'autre ne suffit donc pas à insérer une avalanche équivalente; la correction de la hauteur par $\kappa\doteq\varrho/\varrho'$ reste nécessaire. Un exemple pour $\kappa=1/2$ est donnée à la \autoref{fig:avalanches_equivalentes}.
...
%: leur vitesse peut monter à \SI{60}{\meter\per\second} alors que la hauteur n'est que de quelques mètres. Par exemple, une avalanche de \SI{3}{\meter} de hauteur est en régime supercritique dès que la vitesse dépasse \SI{6}{\meter\per\second} ($\mathrm{Fr}=u/\sqrt{gh}=\num{1,11}>1$). 
