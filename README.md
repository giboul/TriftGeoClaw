...
You can include a dam in the topography by setting the path to the shapefile of a dam in `config.yaml:dams` and its level in `config.yaml:dam_alt` before running `insert_dam.py`.
...
[--color_var -c variable="dh" ] \ # variable is one of h, hu, hv, z, b, dh, ds
[--fgno -n fgno=1] \
